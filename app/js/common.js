$(function() {
            ...
            $(".hamburger").click(function() {
                if ($(".hamburger>.arrow").length == 0) {
                    $(".hamburger>div").addClass("arrow");
                    $(".hamburger").addClass("rotate");
                    $('aside').addClass('open');
                } else {
                    $(".hamburger>div").removeClass("arrow");
            ...
            $('.selectToVin07_options--block select').change(function(e) {
                if ($(this).val() != 0) {
                    console.log('12')
                    $('.selectToVin07_options--block').slideDown()
                } else {
                    $('.selectToVin07_options--hide').slideUp()
                }
            })
            ...
            ////////////////////////
            ///настройки///
            $('#remotePanel').click(function() {
                if ($(this).val() == 3) {
                    $('.generalUserSettings-settingsField_textarea').slideDown()
                } else {
                    $('.generalUserSettings-settingsField_textarea').slideUp()
                }
            });
            ...
            $('#slidedownSettingsAll').click(function() {
                console.log('1');
                if ($(this).val() == 3) {
                    console.log('2');
                    $('#slidedownSettingsAll_select').slideDown();
                } else {
                    $('#slidedownSettingsAll_select').slideUp();
                }
            })
            ...
            //Корзина безнал выбрать все чекбоксы
            $('.cart_beznalField').find('.cart-MarkAll').click(function() {
                    $('.cart_beznalField').find('.cart-checkbox_input').attr('checked', true);
                    $('.cart_beznalField').find('.cart-checkbox').children('.inner').css('backgroundColor', '#50b74e');
                })
                //Корзина безнал очистить все чекбоксы
            $('.cart_beznalField').find('.cart-dellAll').click(function() {
                $('.cart_beznalField').find('.cart-checkbox_input').attr('checked', false);
                $('.cart_beznalField').find('.cart-checkbox').children('.inner').css('backgroundColor', '');
            })

            //Корзина безнал удалить выбранные чекбоксы
            function cartDellMarkNall() { //удалить выделленные
                var $input = $('.cart-beznal')
                    .find('.cart-checkbox_input:checked')
                    .parents('.cart-tableRow').slideUp(200);
            }
            ...
            //Корзина нал выбрать все чекбоксы
            $('.cartnal-MarkAll').click(function() {
                    $('.cart-nalTable').find('.cart-checkbox_input').attr('checked', true);
                    $('.cart-nalTable').find('.cart-checkbox').children('.inner').css('backgroundColor', '#50b74e');
                })
                //Корзина нал очистить все чекбоксы
            $('.cartnal-dellAll').click(function() {
                    $('.cart-nalTable').find('.cart-checkbox_input').attr('checked', false);
                    $('.cart-nalTable').find('.cart-checkbox').children('.inner').css('backgroundColor', '');
                })
                //Корзина безнал удалить выбранные чекбоксы
            function cartDellMarkBezNall() { //удалить выделленные
                var $input = $('.cart-nalTable')
                    .find('.cart-checkbox_input:checked')
                    .parents('.cart-tableRow').slideUp(200);
            }
            $('.cartnal-dellMark').click(cartDellMarkBezNall)
            ...
            $('.top-balance_wrapper strong, .top-balance_curency').click(function() {
                $('.totall--debt').slideToggle(200);
                $('.totall--debt_close').click(function() {
                    $('.totall--debt').slideUp(220);
                });
            });

            // $(window).scroll(function(){
            // 	if($(window).scrollTop() > 80){
            // 		$('.slideDownNav').show().addClass('slideInDown');
            // 		$('.slideDownNav').removeClass('slideInOut');
            // 	}else{
            // 		$('.slideDownNav').addClass('slideInOut');
            // 		$('.slideDownNav').hide().removeClass('slideInDown');
            // 	}
            // });
            ...
            if ($(window).width() > 768) {
                $('.top-balance .totall .totall_wiev').mouseover(balanceToolTip);
                $('.top-balance .totall .totall_wiev').mouseout(balanceToolTipHide);
            }
            ...
            $(".top-balance_update, .finReloaddFinances").on('click', function() {
                ...
                $(this).children().addClass('finReloadd');
                $(this).children().delay(2000);
                $('.top-balance .totall .totall_wiev').css({
                    'color': '#48b746'
                });
                $('.top-balance_update').css('backgroundColor', '#2f933a');
                ...
                $('.balance-tooltip').css({
                    ...
                    'borderColor': 'green'
                }).children('span:first-child').html('Баланс актуален на:' + '<br>' + '<div class="balance-tooltip_date" style="font-family: osb, san-serif; margin:0;">' + d.toLocaleString() + '</div>');
            })
            ...
            //исчезновение кнопки довавить в EXEL
            $('.orders-tableLg_checkbox, .ordersCancellMarckAll, #markAll').click(function() {
                if ($('.orders-tableLg_checkbox').prop('checked')) {
                    $('.orders-table_saveToExel').addClass('green');
                } else {
                    $('.orders-table_saveToExel').removeClass('green');
                }
            });
            ...
            $('.datapicker_orderlist--uncolapse').click(function() { //свернуть фильтры
                var slide = $('.datapicker_orderlist-filterblock');
                var col = $('.datapicker_orderlist--uncolapse');
                col.children('[class*=triangle]').toggleClass('rotate');
            ...
            $(window).on('resize load', function() {
                $('.mob_mnu_left i').hover(function() {
                    if ($('.top-closeMnu').hasClass('flag')) {
            ...
            function mobMnu() {
                ...
                if ($(window).width() < 1400 && $(window).width() >= 768) {
                    ...
                    $('.top-closeMnu, .hamburger-icon').click(function() {

                        $(this).toggleClass('flag');
                        if ($(this).hasClass('flag')) {
                            $('.hidemenuR, .slideDownNav').addClass('menuActiveTo');
                            $('.hidemenuR, .slideDownNav').removeClass('menuActiveFrom');
                        } else {
                            $('.hidemenuR, .slideDownNav').removeClass('menuActiveTo');
                            $('.hidemenuR, .slideDownNav').addClass('menuActiveFrom');
                        }
                    ...
                    $('.top-closeMnu, .hamburger-icon').click(function() {
                        $(this).toggleClass('flag');
                        if ($(this).hasClass('flag')) {
                            $('.hidemenuR, .slideDownNav').addClass('tabletSmenuActiveTo');
                            $('.hidemenuR, .slideDownNav').removeClass('tabletSmenuActiveFrom');
                            $('.mob_logo img').attr('src', 'img/logo.png');

                        } else {
                            $('.hidemenuR, .slideDownNav').removeClass('tabletSmenuActiveTo');
                            $('.hidemenuR, .slideDownNav').addClass('tabletSmenuActiveFrom');
                            $('.mob_logo img').attr('src', 'img/moblogo.png');
                    ...
                    $('.hamburger-icon').click(function() {
                            ...
                            if ($(this).hasClass('flag')) {
                                $('.hidemenuR').animate({'left':'215px'},200);


                            } else {
															$('.hidemenuR').animate({'left':'0px'},200);
                            }
                ...
                function closeCloneElem() {
                    $(this).parents('.generalPopUp-window-cloneElement').slideUp(200);
                    $(this).parents('.generalPopUp-window-cloneElement').delay(200).remove();
                };
